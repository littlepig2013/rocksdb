...
TEST_F(CompactionPickerTest, CompactionPriRoundRobin) {
  ...
  std::vector<InternalKey> next_cursors = {InternalKey("150", 100, kTypeValue),
                                           InternalKey("200", 100, kTypeValue),
                                           InternalKey("200", 100, kTypeValue)};
  ...
  for (size_t i = 0; i < test_cursors.size(); i++) {
    ...
    const std::vector<InternalKey> compact_cursors_by_level =
        vstorage_->GetLeveledCompactCursors();
    ASSERT_EQ(0, icmp_.Compare(compact_cursors_by_level[2], next_cursors[i]));
